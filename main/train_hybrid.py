...
from sentanalyzer.models.hybrid import SentimentAnalyzer
...
CURR_EXP_NO = "exp_hybrid_" + datetime.now().strftime("%dd_%mm_%yy_%HH_%MM")
...
updated_model_file = os.path.join(MODEL_DIR,CURR_EXP_NO,'hybrid.pt')
updated_metric_file = os.path.join(METRIC_DIR,CURR_EXP_NO,'metrics_hybrid.pt')
...
def main():
    ...
    embedding_model = SentimentEmbedding(num_words = 100,vocab_size=vocab_size,
                              output_size=1,
                              embedding_dim=728,
                              )
    weights_matrix = None
    trainer.load_checkpoint(embedding_model,checkpoint)
    for name,param in embedding_model.named_parameters():
        if name=='embedding.weight':
            weights_matrix = param 
            
    model = SentimentAnalyzer(  weights_matrix=weights_matrix,
...
def test():
    ...
    model = SentimentAnalyzer(  weights_matrix=weights_matrix,
                                ...
                                hidden_dim=hidden_dim,
                                num_layers=num_layers)
    model_path = r"E:\Users\VS_Code_Workspace\Python\VirtualEnvironments\sentanalyzer\sentanalyzer\experiments\exp_hybrid_27d_12m_20y_13H_43M\hybrid.pt"
    
    trainer = Trainer()
    trainer.load_checkpoint(model,model_path)
    trainer.test(model,dataset_location,batch_size=batch_size)
